...
const DotField = memo(({
    ...
    function tick() {
      ...
      for (let index = 0; index < dots.length; index += 1) {
        ...
        if (distanceSquared < cursorRadiusSquared && currentEngagement > 0.01) {
          ...
          if (props.bulgeOnly) {
            const intensity = 1 - distance / props.cursorRadius;
            const push = intensity * intensity * props.bulgeStrength * currentEngagement;
        ...
        const radius = props.sparkle && (((index * 2654435761) ^ (frameCount >> 3)) >>> 0) % 100 < 3
          ? dotDrawRadius * 1.8
          : dotDrawRadius;
  ...
  return (
    <div className={`pointer-events-none absolute inset-0 h-full w-full ${className ?? ''}`} {...rest}>
      <canvas
        ref={canvasRef}
        aria-hidden="true"
